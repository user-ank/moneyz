...
export default function Expense() {
  ...
  const [dateMonthSegregatedTransactions, setDateMonthSegregation] = useState<{[month: string] : {[date: string]: Transaction[]}}>({});
 
  const getDateMonthSegregatedTransaction = () => { // segregates transactions of entire year.
    setLoading(true);
    let segregatedTransactions: {[month: string] : {[key: string]: Transaction[] }} = {}
    for (let i = 0; i < transactions.length; i++) {
      let date = transactions[i].dateTime.split(" ")[0];
      let month = date.substring(5,7);
      
      if(month in segregatedTransactions){
        if (date in segregatedTransactions[month]) {
          segregatedTransactions[month][date].push(transactions[i]);
        }
        else {
          segregatedTransactions[month][date] = [transactions[i]];
        }
      }
      else{
        segregatedTransactions[month] = {
          [date]: [transactions[i]],
        };
      }
    }
    setDateMonthSegregation(segregatedTransactions);
    setLoading(false);
  }
  ...
  useEffect(() => {
    db.withTransactionAsync(async () => {
      await getYearTransactions();
    })
  }, [db, year]);

  useEffect(()=>{
    getDateMonthSegregatedTransaction();
    console.log(Object.keys(dateMonthSegregatedTransactions).length);
  }, [transactions]);
  return (
    <>
      {loading ? (
        ...
        <>
          ...
          <ScrollView className="bg-[#fffde8] pt-5" showsVerticalScrollIndicator={false}>
            <View style={styles.container}>
              {(Object.keys(dateMonthSegregatedTransactions).length === 0 || dateMonthSegregatedTransactions[month] === undefined) ? (
                ...
                Object.keys(dateMonthSegregatedTransactions[month.toString()]).map((date, i) => (
                  <View key={i} id="dayTransactions" className="flex self-center w-11/12 mb-7">
                    ...
                    <ThickLineFull />
                    {dateMonthSegregatedTransactions[month.toString()][date].map((transaction, i, arr) => (
